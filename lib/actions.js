  ...
  prisma.$transaction(async (tx) => {
    ...
    for (const slot of slots) {
      ...
      for (const prop in sizeMapping) {
        ...
        sizesData.push(
          sizes.map(
            (size) => ({ size, deviceId: devicedd.id, slotId: createMany.id })))
      }
      ...
      const targetingData = slot?.slotTargeting?.map(
        (item) => ({ ...item, slotId: createMany.id }))
      if (targetingData) {
        await prisma.SlotTargeting.createMany({
            data: targetingData,
          },
        )
      }
